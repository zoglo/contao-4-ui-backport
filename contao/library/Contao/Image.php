...
class Image
{
    private static mixed $htmlTemplateCache;
    /**
	 ...
	 *
	 * @param \Contao\File $file A file instance of the original image
	 *
	 ...
	 */
	public function __construct(\Contao\File $file)
	{
		...
		if (!$file->exists())
		{
			...
			{
				$file = new \Contao\File(StringUtil::stripRootDir($webDir) . '/' . $file->path);
			}
			else
			{
        ...
        if (str_contains($template, '{darkAttributes}'))
        {
            ...
            foreach (array('data-icon', 'data-icon-disabled') as $icon)
            {
                if (isset($darkAttributes[$icon]))
                {
                    ...
                    $darkAttributes[$icon] = $pathinfo['dirname'] . '/' . $pathinfo['filename'] . '--dark.' . $pathinfo['extension'];
                }
            }
	 ...
	 *
	 * @param string|\Contao\File $image The image path or File instance
	...
	public static function create($image, $size=null)
	{
		...
		if (\is_string($image))
		{
			$image = new \Contao\File(rawurldecode($image));
		}
		...
		$fileRecord = \Contao\FilesModel::findByPath($image->path);
	...
	private static function getImageSizeConfig($size)
	{
		if (is_numeric($size))
		{
			return \Contao\ImageSizeModel::findByPk($size);
		}
